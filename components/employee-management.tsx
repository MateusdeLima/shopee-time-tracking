...
export function EmployeeManagement() {
  ...
  const handleTeamChange = async (userId: string, team: string) => {
    try {
      await updateUser(userId, { team: team || undefined })
      // No toast here to avoid spamming while typing, or we can use a debounced approach
      // But for simple direct update, let's just update local state
      setEmployees(prev => prev.map(emp => emp.id === userId ? { ...emp, team } : emp))
    } catch (error) {
      console.error("Error updating team:", error)
    }
  }

  const uniqueTeams = Array.from(new Set(employees.map(emp => emp.team).filter(Boolean)))
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="min-w-[180px]">Time / Equipe</TableHead>
                        ...
                        <TableCell>
                          <div className="flex flex-col gap-1">
                            <Input
                              ...
                              value={employee.team || ""}
                              onChange={(e) => handleTeamChange(employee.id, e.target.value)}
                              list={`teams-list-${employee.id}`}
                              className="h-8 text-xs"
                            />
                            <datalist id={`teams-list-${employee.id}`}>
                              {uniqueTeams.map((team) => (
                                <option key={team as string} value={team as string} />
                              ))}
                            </datalist>
                          </div>
                        </TableCell>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2 text-xs">
                        ...
                        <div className="w-full space-y-2">
                          <Input
                            placeholder="Time / Equipe..."
                            value={employee.team || ""}
                            onChange={(e) => handleTeamChange(employee.id, e.target.value)}
                            list={`teams-list-mobile-${employee.id}`}
                            className="h-8 text-xs w-full"
                          />
                          <datalist id={`teams-list-mobile-${employee.id}`}>
                            {uniqueTeams.map((team) => (
                              <option key={team as string} value={team as string} />
                            ))}
                          </datalist>
